...
## 🎯 1. 概要 (Abstract)
本研究は、江戸時代の黄表紙に漫画の吹き出しの機能があるかをAIを用いて検証する際のAI の挙動を観察から理論化したものである。
大規模言語モデル（LLM）が、特定のトピックは、コンテクストとそれを流し込む「型(KATA)」で構成され、そのトピックに新説が提示され、定説(旧説)型が新説型へ遷移すると、二度と旧説型へは戻らないことを観察した。
